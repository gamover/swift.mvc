...
Application.prototype.setPathToAppConfig = function setPathToAppConfig (pathToAppConfig)
{
    ...
    if (!pathToAppConfig.length)
        throw new $swiftErrors.ValueError('Недопустимое значение пути к конфигурации приложения.');
...
Application.prototype.init = function init (cb)
{
    ...
    this._initCore(function (err)
    {
        if (err)
        {
            ...
            if (!self._disableLog)
            {
                console.log(' - инициализация ядра: ' + $cliColor.red.bold('не выполнена'));

                if (err instanceof $swiftErrors.MultipleError)
                    err.list.forEach(function (error) { console.log('   ' + $cliColor.red(error.message)) });
        ...
        if (!self._disableLog)
            console.log(' - инициализация ядра: ' + $cliColor.green.bold('выполнена'));
        //
        ...
        $swiftUtils.type.isObject(config.swift.dbManager.adapters))
    {
        ...
        function init ()
        {
            ...
            if (errors.length)
            {
                ...
                cb(new $swiftErrors.MultipleError('Ошибки инициализации менеджера баз данных.').setList(errors));
                return;
            }
...
Application.prototype.initHelperManager = function initHelperManager (cb)
{
    ...
    if (self.helperManager)
    {
        ...
        function init ()
        {
            ...
            {
                self._initialized.helpers = false;
                error = new $swiftErrors.SystemError('Ошибка инициализации менеджера помощников (' + err.message + ').').setInfo(err);
            }
...
Application.prototype.initModuleManager = function initModuleManager (cb)
{
    ...
    function init ()
    {
        ...
        {
            self._initialized.modules = false;
            error = new $swiftErrors.SystemError('Ошибка инициализации менеджера модулей (' + err.message + ').').setInfo(err);
        }
...
Application.prototype.initServer = function initModuleManager (cb)
{
    ...
    function init ()
    {
        ...
        {
            self._initialized.server = false;
            error = new $swiftErrors.SystemError('Ошибка инициализации сервера (' + err.message + ').').setInfo(err);
        }
                ...
                self.runModules(function (err)
                {
                    ...
                    self.runServer(function (err)
                    {
                        ...
                        if (errors.length)
                        {
                            ...
                            cb(new $swiftErrors.MultipleError('Ошибки запуска приложения.').setList(errors));
                            return;
                        }
