...
Application.prototype.extra = function extra (key, value)
{
    if (typeof value !== 'undefined')
    {
        ...
        {
            throw new $swiftErrors.TypeError('не удалось добавить параметр. Имя параметра представлено в недопустимом формате');
        }
        if (!key.length)
        {
            throw new $swiftErrors.TypeError('не удалось добавить параметр. Не задано имя параметра');
        }
    }
    else
    {
        if (typeof key !== 'string')
        {
            throw new $swiftErrors.TypeError('не удалось получить значение параметра. Имя параметра представлено в недопустимом формате');
        }
        if (!key.length)
        {
            throw new $swiftErrors.TypeError('не удалось получить значение параметра. Не задано имя параметра');
        }
    }
            ...
            function init ()
            {
                ...
                Object.keys(cAdapters).forEach(function (adapterType)
                {
                    ...
                    try
                    {
                        ...
                        if (!$swiftUtils.type.isObject(cConnections))
                        {
                            return;
                        }
                        ...
                        Object.keys(cConnections).forEach(function (connectionName)
                        {
                            ...
                            {
                                errors.push(new $swiftErrors.SystemError()
                                    .setMessage('не удалось создать соединение "' + connectionName + '"(' +
                                        err.message + ')')
                                    .setInfo(err));
                            }
                        });
                    ...
                    {
                        errors.push(new $swiftErrors.SystemError()
                            .setMessage('не удалось создать адаптер "' + adapterName + '"(' + err.message + ')')
                            .setInfo(err));
                    }
                });
            ...
            function init ()
            {
                ...
                Object.keys(cLoggers).forEach(function (loggerName)
                {
                    try
                    {
                        ...
                        //
                        if (typeof cLogger.path === 'string' && cLogger.path.length)
                        {
                            ...
                            else if (pathToLogArr[0] !== '/') pathToLogArr.unshift(config.swift.path.project);
            ...
            if (Object.keys(dbAdapters).length)
            {
                ...
                {if (!dbAdapters.hasOwnProperty(adapterName)) continue;
                    var adapter = dbAdapters[adapterName],
                        connections = adapter.getAllMongoConnections();
                    ...
                    {if (!connections.hasOwnProperty(connectionName)) continue;
                        ...
                        (function (connectionName, connection)
                        {
                            ...
                            connection.connect(function (err)
                            {
                                if (err)
                                {
                                    if (!self._disableLog)
                                    {
                                        console.log('     соединение "' + $cliColor.blue.bold(connectionName) + '" (' + connection.getUri() + '): ' + $cliColor.red.bold('ошибка'));
                                        console.log('     ' + $cliColor.red(err.message));
                                    }
                                    if (!error) error = err;
                                    threads--;
                                ...
                                if (!self._disableLog)
                                    console.log('     соединение "' + $cliColor.blue.bold(connectionName) + '" (' + connection.getUri() + '): ' + $cliColor.green.bold('установлено'));
                ...
                (function (moduleName)
                {
                    ...
                    modules[moduleName].run(function (err)
                    {
                        if (err)
                        {
                            if (!self._disableLog)
                            {
                                console.log('    модуль "' + $cliColor.blue.bold(moduleName) + '" (' + modul.getPathToModule().replace(self.configurator.getConfig().swift.path.project, '') + '): ' + $cliColor.red.bold('ошибка'));
                                console.log($cliColor.red('    ' + err.message));
                            }
                            if (!error) error = err;
                            threads--;
                        ...
                        if (!self._disableLog)
                            console.log('    модуль "' + $cliColor.blue.bold(moduleName) + '" (' + modul.getPathToModule().replace(self.configurator.getConfig().swift.path.project, '') + '): ' + $cliColor.green.bold('запущен'));
